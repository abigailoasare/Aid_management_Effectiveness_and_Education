...
overview <- read_dta(file.path(dir[["gps_raw"]] , "overview.dta"))
...
out_shapefile <- lapply(list_shp, function(x) {
  file <- sf::read_sf(paste(dir[["gps_raw"]], x, sep = "/")) %>%
...
st_write(
  dhs_clusters,
  dsn = file.path(dir[["data-processed"]],"dhs_clusters.shp"),
  layer = "dhs_clusters.shp",
  driver = "ESRI Shapefile",
  append = FALSE
)

write_csv(dhs_clusters, file.path(dir[["data-processed"]], "dhs_clusters.csv"))
