...
overview <- read_dta(file.path(dir[["pr_raw"]], "overview.dta"))
...
out <- lapply(list_dta, function(x) {
  file <- read_dta(paste(dir[["pr_raw"]], x, sep = "/")) %>%
    as_tibble() %>%
...
expss::write_labelled_csv(dhs_full_1, file.path(dir[["data-processed"]], "dhs_full.csv"))
